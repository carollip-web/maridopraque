...
-- Backfill: pedidos já aprovados/pagos que ficaram sem data_agendada
UPDATE public.orcamentos
   SET data_agendada = (data_preferida::timestamp + COALESCE(horario_preferido, time '09:00')) AT TIME ZONE 'America/Sao_Paulo',
       updated_at = now()
 WHERE data_agendada IS NULL
   AND data_preferida IS NOT NULL
