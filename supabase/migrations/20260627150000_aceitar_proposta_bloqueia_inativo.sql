-- Garante que o cliente não consiga aceitar a proposta de um profissional
-- INATIVO (ativo = false). Recria as duas assinaturas de aceitar_proposta_cliente
-- mantendo o fluxo atual (agenda "confirmado") e adicionando a verificação.
...
BEGIN
  ...
  -- Não permite aceitar proposta de profissional inativo.
  IF EXISTS (
    SELECT 1 FROM public.profissional_perfil
    WHERE user_id = v_prop.profissional_id AND ativo = false
  ) THEN
    RAISE EXCEPTION 'Este profissional está indisponível no momento. Escolha outra proposta.';
  END IF;
