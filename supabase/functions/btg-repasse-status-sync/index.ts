...
serve(async (req) => {
  ...
  try {
    ...
    if (!repasses || repasses.length === 0) {
      return new Response(
        JSON.stringify({ results: [], message: "Nenhum repasse pendente de sincronização." }),
        {
          headers: { ...corsHeaders, "Content-Type": "application/json" },
          status: 200,
        },
    ...
    if (tokenExpired || !hasScope) {
      return new Response(
        JSON.stringify({
          error: "Token BTG sem permissão de consulta ou expirado. Reconecte o BTG.",
        }),
        {
          status: 401,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
      ...
      try {
        ...
        if (paymentsData.length === 0) {
          results.push({
            id: repasse.id,
            success: false,
            error: "Lote não retornou pagamentos na API",
          });
          continue;
        }
        ...
        if (["CONFIRMED", "PAID", "LIQUIDATED"].includes(btgStatus)) {
          ...
          ["FAILED", "REJECTED", "ERROR", "CANCELED", "REVERTED", "ADJOURNED"].includes(btgStatus)
        ) {
          nextStatus = "erro";
