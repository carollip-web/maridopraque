...
serve(async (req) => {
  ...
  try {
    ...
    const { data: repasse, error: repasseErr } = await supabaseAdmin
      ...
      .select("*, profissional_perfil:profissional_id(pix_key, pix_key_type, pix_holder_name, pix_holder_document)")
      .eq("id", repasse_id)
      .single()
    ...
    if (expiresAt.getTime() - 60000 < Date.now()) {
      ...
      await supabaseAdmin.from("marketplace_integracoes").update({
