...
serve(async (req) => {
  ...
  try {
    ...
    if (!dryRun && !confirmRealPayment) {
      return new Response(
        JSON.stringify({ error: "Repasse real bloqueado: confirmação explícita ausente." }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (!btgConfig.company_id || !btgConfig.account_id) {
      return new Response(
        JSON.stringify({
          error: "Integração BTG conectada, mas conta/empresa ainda não configurada.",
        }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
    ...
    if (tokenExpired || !hasScope) {
      return new Response(
        JSON.stringify({ error: "Token BTG sem permissão de pagamento ou expirado." }),
        {
          status: 401,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
      ...
      try {
        ...
        if (repError || !repasse) {
          results.push({
            id: repasseId,
            success: false,
            error: "Repasse não encontrado ou não está no status 'aprovado'.",
          });
          continue;
        }
        ...
        if (dryRun) {
          results.push({
            ...
            dryRun: true,
            message:
              "Dry-run concluído com sucesso. Integração válida, scopes verificados e payload sanitizado.",
