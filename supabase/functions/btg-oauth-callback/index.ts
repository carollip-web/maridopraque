...
serve(async (req) => {
  ...
  try {
    ...
    if (fetchError || !integrationData) {
      return new Response(JSON.stringify({ error: "Sessão de autenticação não encontrada ou expirada" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      })
    }
    ...
    if (!savedState || savedState !== state) {
      return new Response(JSON.stringify({ error: "Estado de autenticação inválido (CSRF detectado)" }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      })
    }
    ...
    if (!tokenResponse.ok) {
      ...
      return new Response(JSON.stringify({ error: `Erro no servidor do BTG ao obter token: ${errBody}` }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      })
    }
