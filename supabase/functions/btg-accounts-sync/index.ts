...
serve(async (req) => {
  ...
  try {
    ...
    if (tokenExpired) {
      return new Response(JSON.stringify({ error: "Token BTG sem permissão para consultar contas ou expirado." }), {
        status: 401,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      })
    }
    ...
    if (!companyId) {
      return new Response(JSON.stringify({ error: "Não foi possível identificar o companyId no token." }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      })
    }
    ...
    if (!btgResponse.ok) {
      ...
      return new Response(JSON.stringify({ error: "Erro ao consultar BTG.", details: responseJson }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      })
    }
    ...
    if (accounts.length === 0) {
      return new Response(JSON.stringify({ error: "Nenhuma conta BTG encontrada para este token." }), {
        status: 404,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      })
    }
    ...
    if (account_id) {
       selectedAccount = accounts.find((a: any) => a.number === account_id || a.accountId === account_id)
    ...
    if (selectedAccount) {
      const updatePayload = {
        ...
        extra: { ...(btgConfig.extra as any || {}), accounts_raw: accounts, selected_account: selectedAccount }
      }
      await supabaseAdmin
        .from("marketplace_integracoes")
