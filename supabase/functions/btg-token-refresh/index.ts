...
serve(async (req) => {
  try {
    ...
    if (dbErr || !btgData || !btgData.refresh_token) {
      ...
      return new Response(JSON.stringify({ ok: true, msg: "Nenhuma ação necessária" }), { status: 200 })
    }
    ...
    const { access_token, refresh_token: new_refresh_token, expires_in, scope, account_id, company_id } = tokenData
