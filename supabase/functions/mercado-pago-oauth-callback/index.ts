...
serve(async (req) => {
  ...
  try {
    ...
    if (!mpAppId || !mpClientSecret) {
      ...
      return new Response(
        JSON.stringify({ error: "Configuração do Mercado Pago ausente no servidor" }),
        {
          status: 500,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    if (!tokenResponse.ok) {
      ...
      return new Response(
        JSON.stringify({ error: `Erro ao obter token do Mercado Pago: ${errBody}` }),
        {
          status: 400,
          headers: { ...corsHeaders, "Content-Type": "application/json" },
        },
    ...
    await supabaseAdmin.from("notificacoes").insert({
      ...
      mensagem:
        "Sua conta do Mercado Pago foi vinculada com sucesso! Você já pode receber pagamentos com split de comissão.",
