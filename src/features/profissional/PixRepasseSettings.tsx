  ...
  const loadProfile = async () => {
    try {
      ...
      const { data, error } = await supabase
        ...
        .select("pix_key_type, pix_key, pix_holder_name, pix_holder_document, pix_dados_confirmados, repasse_automatico")
        .eq("user_id", user.id)
        .maybeSingle()
    ...
    <Card className="p-6 space-y-6 border-border shadow-sm bg-card transition-all duration-300 hover:shadow-md">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="space-y-1">
          <div className="flex items-center gap-2">
            ...
            <h3 className="text-lg font-semibold tracking-tight text-foreground">Dados Pix para Repasse</h3>
          </div>
        ...
        <div>
          {profile?.pix_dados_confirmados ? (
            <Badge variant="secondary" className="bg-emerald-50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-400 border-emerald-200 dark:border-emerald-800 gap-1.5 py-1 px-3">
              <CheckCircle2 className="h-3.5 w-3.5" />
              Pix confirmado
            </Badge>
          ) : (
            <Badge variant="outline" className="text-amber-600 dark:text-amber-400 border-amber-200 dark:border-amber-800 gap-1.5 py-1 px-3">
      ...
      <div className="rounded-xl bg-primary/5 dark:bg-primary/10 border border-primary/20 p-4 flex gap-3 text-sm">
        ...
        <div className="text-primary-foreground/90 dark:text-primary-foreground/80 space-y-1">
          <p className="font-semibold text-primary dark:text-primary-foreground">Aviso Importante</p>
          <p className="text-xs text-muted-foreground leading-relaxed">
            Esses dados serão usados para enviar seus repasses Pix após a conclusão dos serviços. Certifique-se de que a chave pertence ao titular informado.
          </p>
        </div>
      </div>
      ...
      <form onSubmit={handleSave} className="space-y-4">
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            <Label htmlFor="pix_key_type" className="text-sm font-medium">Tipo da chave Pix</Label>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div className="space-y-2">
            <Label htmlFor="pix_holder_name" className="text-sm font-medium">Nome do titular da conta</Label>
            <Input
              id="pix_holder_name"
              type="text"
          ...
          <div className="space-y-2">
            <Label htmlFor="pix_holder_document" className="text-sm font-medium">CPF/CNPJ do titular</Label>
            <Input
        ...
        <div className="flex items-center justify-between rounded-lg border border-border p-4 bg-muted/30">
          <div className="space-y-0.5">
            <Label htmlFor="repasse_automatico" className="text-sm font-semibold cursor-pointer">Repasse automático</Label>
            <p className="text-xs text-muted-foreground">
              Enviar o repasse Pix automaticamente assim que o serviço for finalizado e pago.
            </p>
          </div>
          <Switch
            id="repasse_automatico"
            checked={autoRepasse}
            onCheckedChange={setAutoRepasse}
          />
        </div>

        <div className="flex justify-end pt-2">
          <Button type="submit" disabled={saving} className="w-full sm:w-auto h-10 px-6 font-semibold">
            {saving ? (
