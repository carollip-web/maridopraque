...
async function assertSuperAdmin(supabase: any, userId: string) {
  const { data: roles } = await supabase
    ...
    .eq("user_id", userId);
  const isSuper = roles?.some((r: any) => r.role === "admin" && r.admin_level === "super_admin");
  if (!isSuper) {
    throw new Error("Apenas super administradores podem gerenciar a equipe.");
  }
}
...
export const convidarAdmin = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .inputValidator((input) => inviteAdminSchema.parse(input))
  .handler(async ({ data, context }) => {
    ...
    if (!targetUserId) {
      ...
      if (authErr || !authUser.user) {
        throw new Error(`Erro ao criar conta: ${authErr?.message || "desconhecido"}`);
      }
      targetUserId = authUser.user.id;
      created = true;
      // Ensure profile exists with name + email
      await admin.from("profiles").upsert({ id: targetUserId, nome, email });
    }
    ...
    const { error: roleErr } = await admin
      .from("user_roles")
      .upsert(
        { user_id: targetUserId, role: "admin", admin_level: data.admin_level },
        { onConflict: "user_id,role" },
      );
    if (roleErr) throw new Error(`Erro ao atribuir papel: ${roleErr.message}`);

    return { ok: true, userId: targetUserId, created, tempPassword };
  });
