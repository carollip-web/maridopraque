  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on("postgres_changes",
        ...
        (payload) => {
          setMsgs((prev) => prev.some((m) => m.id === (payload.new as Mensagem).id) ? prev : [...prev, payload.new as Mensagem]);
        })
      .subscribe();
  ...
  return (
    <div className="flex flex-col h-[500px] max-h-[70vh] rounded-2xl border border-border bg-white overflow-hidden">
      <div className="px-4 py-3 border-b border-border bg-slate-50">
        <p className="text-sm font-bold text-slate-900">Conversa{contraparteNome ? ` com ${contraparteNome}` : ""}</p>
      </div>

      <div className="flex-1 overflow-y-auto p-4 space-y-2 bg-slate-50/50">
        {loading ? (
          <div className="grid place-items-center h-full"><Loader2 className="h-6 w-6 animate-spin text-brand" /></div>
        ) : msgs.length === 0 ? (
          <p className="text-xs text-muted-foreground text-center py-8">Inicie a conversa enviando uma mensagem abaixo.</p>
        ) : (
          msgs.map((m) => {
            ...
            return (
              <div key={m.id} className={cn("flex", mine ? "justify-end" : "justify-start")}>
                <div className={cn(
                  ...
                  mine ? "bg-brand text-white rounded-br-sm" : "bg-white border border-border rounded-bl-sm"
                )}>
                  <p className="whitespace-pre-wrap break-words">{m.texto}</p>
                  <p className={cn("text-[10px] mt-1", mine ? "text-white/70" : "text-muted-foreground")}>
      ...
      >
        <Input
          ...
          disabled={enviando}
        />
        <Button type="submit" disabled={!texto.trim() || enviando} className="bg-brand hover:bg-brand-700 rounded-full">
