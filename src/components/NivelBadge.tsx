...
export function NivelBadge({
  ...
  if (compact) {
    return (
      <span
        className={cn(
          "inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-gradient-to-r",
          nivel.cor,
        )}
      >
    ...
    <div className={cn("p-4 rounded-2xl bg-gradient-to-br shadow-sm", nivel.cor)}>
      ...
      {nivel.proximo && (
        <p className="text-[11px] mt-2 opacity-90">
          ...
          {nivel.proximo.faltam.servicos
            ? ` · faltam ${nivel.proximo.faltam.servicos} serviços`
            : ""}
          {nivel.proximo.faltam.nota && nivel.proximo.faltam.nota > 0
            ? ` · subir nota em ${nivel.proximo.faltam.nota.toFixed(1)}`
            : ""}
        </p>
      )}
    </div>
