...
export function AdminModoTeste() {
  ...
  const handleSeed = async () => {
    ...
    try {
      ...
      toast.success(`Seed concluído: ${r.contas?.length ?? 0} contas, ${r.pedidos_criados ?? 0} pedidos.`);
  ...
  const handleReset = async () => {
    if (!confirm("Apagar TODOS os dados de teste (usuários e pedidos)? Esta ação não pode ser desfeita.")) return;
    setResetting(true);
    try {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-muted-foreground mt-1">
          Crie dados de teste, simule fluxos e valide integrações entre cliente, profissional e admin.
          Senha padrão: <code className="bg-slate-100 px-1.5 py-0.5 rounded">Teste@2026!</code>
        </p>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-border p-6">
        ...
        {!contas?.length ? (
          <p className="text-sm text-muted-foreground">Nenhuma conta de teste. Clique em "Criar dados de teste".</p>
        ) : (
          <div className="space-y-2">
            {contas.map((c: any) => (
              <div key={c.id} className="flex items-center justify-between gap-3 p-3 bg-slate-50 rounded-lg">
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-bold truncate">{c.nome}</p>
                  <p className="text-xs text-muted-foreground truncate">{c.email}</p>
                </div>
                <span className="text-[10px] font-bold uppercase px-2 py-1 rounded bg-brand/10 text-brand">{c.role}</span>
                <button
            ...
            {pedidos.map((p: any) => (
              <div key={p.id} className="p-3 bg-slate-50 rounded-lg">
                <div className="flex items-center justify-between gap-3 mb-2">
                  ...
                  <span className="text-[10px] font-bold uppercase px-2 py-1 rounded bg-amber-100 text-amber-800">{p.status}</span>
                </div>
                <div className="flex flex-wrap gap-1.5">
                  {p.status === "customizado_pendente" && (
                    <ActionBtn busy={advancing === p.id + "enviado"} onClick={() => advance(p.id, "enviado", { valor: 250, valor_servico: 250 })}>
                      → Enviar orçamento
                    </ActionBtn>
                  )}
                  {p.status === "enviado" && (
                    <ActionBtn busy={advancing === p.id + "aprovado"} onClick={() => advance(p.id, "aprovado", { data_aprovacao: new Date().toISOString() })}>
                      → Aprovar
                    </ActionBtn>
                  )}
                  {p.status === "aprovado" && (
                    <ActionBtn busy={advancing === p.id + "agendado"} onClick={() => advance(p.id, "agendado", { data_agendada: new Date(Date.now() + 86400000).toISOString() })}>
                      → Agendar (amanhã)
                    </ActionBtn>
                  )}
                  {(p.status === "aprovado" || p.status === "agendado") && (
                    <ActionBtn busy={advancing === p.id + "pago"} onClick={() => advance(p.id, "pago", { data_pagamento: new Date().toISOString() })}>
                      → Marcar pago
                    </ActionBtn>
                  )}
                  {p.status === "pago" && (
                    <ActionBtn busy={advancing === p.id + "concluido"} onClick={() => advance(p.id, "concluido")}>
                      → Concluir
                    </ActionBtn>
                  )}
                </div>
