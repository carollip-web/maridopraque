...
import {
  ArrowLeft, Loader2, CheckCircle2, XCircle, Clock, Eye,
  User, MapPin, FileText, ChevronDown, ChevronUp, Search,
...
function AdminValidacao() {
  ...
  const handleAprovar = async () => {
    if (!selected) return;
    setSaving(true);
    const { error } = await (supabase.from("profissional_perfil") as any).update({
  ...
  const handleMarcarAnalise = async (p: Prestador) => {
    await (supabase.from("profissional_perfil") as any).update({ aprovacao_status: "em_analise" }).eq("user_id", p.user_id);
    refresh();
  };
  ...
  if (!isAdmin) return <div className="max-w-md mx-auto py-32 text-center"><h1 className="text-2xl font-bold">Acesso restrito</h1></div>;
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-10">
        ...
        <div className="flex items-center justify-between mb-8 flex-wrap gap-3">
          <div>
            ...
            <p className="text-muted-foreground mt-1 text-sm">Analise e aprove os cadastros de prestadores de serviço.</p>
          </div>
          <Link to="/profissional-cadastro">
            <Button variant="outline" className="rounded-full gap-2 text-sm">Ver formulário de cadastro</Button>
          </Link>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
          {(["em_analise", "pendente", "aprovado", "rejeitado"] as const).map((s) => {
            ...
            const Icon = cfg.icon;
            return (
              <button key={s} onClick={() => setFilterStatus(s === filterStatus ? "todos" : s)}
                ...
                <div className={`h-9 w-9 rounded-xl flex items-center justify-center mb-2 ${cfg.bg}`}>
        ...
        <div className="grid lg:grid-cols-5 gap-6">
          ...
          <div className="lg:col-span-2 space-y-3">
            <div className="bg-white rounded-2xl border border-slate-200 p-3 shadow-sm flex gap-2">
              ...
              <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Buscar prestador..." className="flex-1 text-sm bg-transparent outline-none" />
            </div>
            {loadingList ? (
                ...
                <button key={p.user_id} onClick={() => setSelected(p)}
                  ...
                  <div className="flex flex-wrap gap-1">
                    {p.especialidades.slice(0, 3).map((e: string) => (
                      <span key={e} className="text-[10px] bg-slate-100 text-slate-600 px-2 py-0.5 rounded-full font-medium">{e}</span>
                    ))}
                    {p.especialidades.length > 3 && <span className="text-[10px] text-muted-foreground">+{p.especialidades.length - 3}</span>}
                  </div>
          ...
          <div className="lg:col-span-3">
            {!selected ? (
              <div className="py-24 text-center bg-white rounded-2xl border border-dashed border-slate-300">
                ...
                <p className="text-slate-500 font-medium text-sm">Selecione um cadastro para analisar.</p>
              </div>
            ) : (
                ...
                <div className="px-6 py-5 border-b border-border flex items-center justify-between flex-wrap gap-3">
                  ...
                  <div className="flex gap-2 items-center flex-wrap">
                    ...
                    {selected.aprovacao_status === "pendente" && (
                      <Button size="sm" variant="outline" className="rounded-full text-xs" onClick={() => handleMarcarAnalise(selected)}>
                        Iniciar análise
                      </Button>
                    )}
                  </div>
                ...
                <div className="p-6 space-y-6 max-h-[calc(100vh-280px)] overflow-y-auto">
                  ...
                  <Section title="Dados pessoais" icon={User}>
                    <Grid2>
                      ...
                      <Field label="Nascimento" value={selected.data_nascimento ? new Date(selected.data_nascimento).toLocaleDateString("pt-BR") : null} />
                      <Field label="Telefone" value={selected.telefone} />
                      <Field label="Experiência" value={selected.experiencia_anos ? `${selected.experiencia_anos} anos` : null} />
                    </Grid2>
                  </Section>

                  {/* Address */}
                  <Section title="Endereço" icon={MapPin}>
                    <Field label="Endereço completo" value={[selected.endereco, selected.numero, selected.bairro, selected.cidade, selected.estado].filter(Boolean).join(", ")} />
                  </Section>
                  ...
                  <Section title="Experiência" icon={FileText}>
                    <div className="flex flex-wrap gap-1.5 mb-3">
                      {selected.especialidades.map((e) => (
                        <span key={e} className="text-xs bg-brand/10 text-brand px-2.5 py-1 rounded-full font-medium">{e}</span>
                      ))}
                    </div>
                    {selected.bio && <p className="text-sm text-slate-700 leading-relaxed bg-slate-50 rounded-xl p-4">{selected.bio}</p>}
                    {selected.observacoes_cadastro && <p className="text-xs text-muted-foreground mt-2">{selected.observacoes_cadastro}</p>}
                  </Section>
                        ...
                        <div key={label}>
                          ...
                          {url ? (
                            <a href={url} target="_blank" rel="noopener noreferrer">
                              <img src={url} alt={label} className="w-full h-28 object-cover rounded-xl border border-border hover:opacity-80 transition-opacity" onError={(e: React.SyntheticEvent<HTMLImageElement>) => { (e.target as HTMLImageElement).style.display = "none"; }} />
                  ...
                  {selected.motivo_rejeicao && (
                    <div className="p-4 rounded-xl bg-red-50 border border-red-200">
                      <p className="text-xs font-bold uppercase text-red-700 mb-1">Motivo da rejeição anterior</p>
                  ...
                  {(selected.aprovacao_status as string) !== "aprovado" && (
                    <div className="space-y-3 pt-2 border-t border-border">
                      {selected.aprovacao_status !== "aprovado" && (
                        <Button onClick={handleAprovar} disabled={saving} className="w-full bg-green-600 hover:bg-green-700 text-white rounded-xl font-bold gap-2">
                          {saving ? <Loader2 className="h-4 w-4 animate-spin" /> : <CheckCircle2 className="h-4 w-4" />} Aprovar prestador
                        </Button>
                      )}
                      <div className="space-y-2">
                        <textarea value={motivo} onChange={(e) => setMotivo(e.target.value)} rows={2} placeholder="Motivo da rejeição (obrigatório para rejeitar)..." className="w-full px-4 py-3 rounded-xl border border-border bg-slate-50 text-sm resize-none" />
                        <Button onClick={handleRejeitar} disabled={saving || !motivo.trim()} variant="outline" className="w-full border-red-300 text-red-600 hover:bg-red-50 rounded-xl font-bold gap-2">
                  ...
                  {selected.aprovacao_status === "aprovado" && (
                    <div className="p-4 rounded-xl bg-green-50 border border-green-200 text-sm text-green-800 font-medium">
                      ✓ Aprovado em {selected.aprovado_em ? new Date(selected.aprovado_em).toLocaleDateString("pt-BR") : "—"}
                    </div>
                  )}
                </div>
              </div>
